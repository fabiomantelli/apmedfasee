function varargout = RBE(varargin)
% RBE MATLAB code for RBE.fig
%      RBE, by itself, creates a new RBE or raises the existing
%      singleton*.
%
%      H = RBE returns the handle to a new RBE or the handle to
%      the existing singleton*.
%
%      RBE('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in RBE.M with the given input arguments.
%
%      RBE('Property','Value',...) creates a new RBE or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before RBE_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to RBE_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help RBE

% Last Modified by GUIDE v2.5 14-Jan-2015 17:17:30

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @RBE_OpeningFcn, ...
    'gui_OutputFcn',  @RBE_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before RBE is made visible.
function RBE_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to RBE (see VARARGIN)

% Choose default command line output for RBE
...
set(handles.text17,'String',taxa_amos);
...
set(handles.edit5,'String',20); % Ordem do Modelo
set(handles.edit6,'String',80); % Número de blocos linha
if (tempo(length(tempo)))>600
    set(handles.edit7,'String',600);           % Tamanho da janela em segundos
    set(handles.edit8,'String',600*taxa_amos); % Tamanho da janela em amostras
    set(handles.edit9,'String',60);            % Passo da janela em segundos
    set(handles.edit10,'String',60*taxa_amos); % Passo da janela em amostras
else
    set(handles.edit7,'String',60);            % Tamanho da janela em segundos
    set(handles.edit8,'String',60*taxa_amos);  % Tamanho da janela em amostras
    set(handles.edit9,'String',5);             % Passo da janela em segundos
    set(handles.edit10,'String',5*taxa_amos);  % Passo da janela em amostras
end
set(handles.edit11,'String',0.1); % Frequência mínima
set(handles.edit12,'String',3.0); % FrequÊncia máxima

% UIWAIT makes RBE wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = RBE_OutputFcn(hObject, eventdata, handles)
...
aux1 = aux*taxa_amos;
if aux1 > str2num(get(handles.edit4,'String')) || aux < 1; aux=600; set(handles.edit7,'String',num2str(aux));end
...
global terminais_qtde selecao terminais_cor taxa_amos sinal  legenda terminal_nome tempo_legenda fator_tempo %unidade sinal_nome

t1 = str2num(get(handles.edit3,'String')); % Amostra inicial
t2 = str2num(get(handles.edit4,'String')); % Amostra final
n  = str2num(get(handles.edit5,'String')); % Ordem do Modelo
k = str2num(get(handles.edit6,'String')); % Número de blocos linha
J = str2num(get(handles.edit7,'String')); % Tamanho da janela em segundos
passo = str2num(get(handles.edit9,'String')); % Passo da janela em segundos
banda(1) = str2num(get(handles.edit11,'String')); % Frequência mínima
banda(2) = str2num(get(handles.edit12,'String')); % FrequÊncia máxima
...
for i=1:1:terminais_qtde
    if selecao(i)==1 % verifica se foi escolhida para plotar
        if get(handles.checkbox1,'value') == 1; % Tira a tendência
            sinalP(ii,1:(t2-t1)+1) = dtrend(sinal(i,t1:t2));
        else
            sinalP(ii,1:(t2-t1)+1) = sinal(i,t1:t2);
        end
        ii=ii+1;
    end
end
...
% Método RBE Multissinais
%--------------------------------------------------------------------------
% Técnicas de Janelamento % Isso aqui tranca o programa e não serve pra
% nada
% Janela Rampa
% Xr=0:taxa_amos:J;
% Yr=1/J*Xr;
...
Ntot=length(sinal(t1:t2));

ini_control=1;           % determina o primeiro ponto a ser analisado
fim_control=J*taxa_amos; % determina o último ponto a ser analisado
...
[p,Ndat] = size(sinalP(:,ini_control:fim_control)); %p=número de dados, Ndat número de sinais
N = Ndat-2*k;
Y=[];
ii = 0;
for i = 1:p:2*k*p-p+1 
    ii = ii+1; 
   Y(i:i+p-1,:) = sinalP(:,ini_control+ii:ini_control+ii+N-1);
end
...
% Montagem do sistema linear para o cálculo dos resíduos.
z=eig(A); % FEITOOOOOOOO!!!!!!!!!!!!!!!!!!!!!!!! 

% Cálculo dos valores de Freq, amortecimento e amplitude

% Monta a matriz Z que é semelhante a todos os sinais
Z=[];
...
%modos=1:size(z); % indices do número de modos
amos=1/taxa_amos;
z_s=log(z)/amos;
amort = (-real(z_s)./(real(z_s).^2+imag(z_s).^2).^0.5)*100;
freq = angle(z)./(2*pi*amos);
...
for i=1:size(sinal)
 Y_res(:,i)=sinal(ini_control:ini_control+Ndat-1);

% Resolve o sistema Linear igual para todos os métodos
 res=Z*Y_res(:,i); %para economizar PC, Z=Pinv(Z)
 ampl(:,i) = abs(res);

end

% fazer uma amplitude média de todos os sinais, isso pode não ser muito
% bom.
ampl=sum(ampl,2)/size(ampl,2);


% Nesse ponto já é conhecido todos os valores de amplitude, amortecimento e
% amplitude, é preciso então encontrar as faixas

num_banda=size(banda,1);

for i=1:num_banda %executará as plotagens
   
    loc = freq>=banda(i,1) & freq<=banda(i,2); % retorna 0 onde for fora da faixa e 1 dentro da faixa
    ...
    hold on
    subplot(2,1,1)
    plot(((t1+fim_control)/taxa_amos)/fator_tempo,0,'ro')
    title('Frequência')
    hold on
    subplot(2,1,2)
    plot(((t1+fim_control)/taxa_amos)/fator_tempo,0,'ro')
    ...
    ampl2=ampl.*loc; % mantém somente as amplitudes dentro da faixa, pois serão mult por 1, as demais por 0
    loc = find(ampl2==max(ampl2)); %identifica qual o local que está essa amplitude, e será utilizado na frequência
    
    
    figure(i)
    hold on
    subplot(2,1,1)
    plot(((t1+fim_control)/taxa_amos)/fator_tempo,freq(loc),'b*')
    title('Frequência')
    hold on
    subplot(2,1,2)
    plot(((t1+fim_control)/taxa_amos)/fator_tempo,amort(loc),'b*')
    title('Amortecimento')
    xlabel(tempo_legenda); %isso pode ser feito somente no final economizando tempo
    end
...
%Novo Passo
ini_control=ini_control+passo*taxa_amos;
fim_control=fim_control+passo*taxa_amos;
%i=i+1;
end %loop das janelas deslizantes
...
% pol = [1 -a.'];          %polinômio z-[a(1)z^n-1+a(2)z^n-2+...+a(n)]=0
% raizes = roots(pol);                             % raízes PLANO Z
...
% for i=1:p
%     if  freq(i)<F && freq(i)>0.009 % || max(ampl(:,i))<0.01 || taxa_amor(i)>40
%         m = m+1;
...
% disp ('===================================================================================================')
% disp ('                               Método de Prony (Mono e Multissinais)                               ')
% disp ('===================================================================================================')
% disp ('                                       DADOS DE SAIDA                                              ')
% fprintf('Ordem do Modelo [N/4]......: %g  \n',p)
...
for i=1:1:terminais_qtde
    if selecao(i)==1 % verifica se foi escolhida para plotar
        if get(handles.checkbox1,'value') == 1; % Tira a tendência
            H = plot(tempo(t1:t2)/fator_tempo,dtrend(sinal(i,t1:t2)));
        else
            H = plot(tempo(t1:t2)/fator_tempo,sinal(i,t1:t2));
        end;

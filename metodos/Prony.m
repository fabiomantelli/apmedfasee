function varargout = Prony(varargin)
% Prony MATLAB code for Prony.fig
%      Prony, by itself, creates a new Prony or raises the existing
%      singleton*.
%
%      H = Prony returns the handle to a new Prony or the handle to
%      the existing singleton*.
%
%      Prony('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in Prony.M with the given input arguments.
%
%      Prony('Property','Value',...) creates a new Prony or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Prony_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Prony_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Prony

% Last Modified by GUIDE v2.5 17-Jun-2014 10:37:20

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @Prony_OpeningFcn, ...
    'gui_OutputFcn',  @Prony_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before Prony is made visible.
function Prony_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Prony (see VARARGIN)

% Choose default command line output for Prony
...
% UIWAIT makes Prony wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = Prony_OutputFcn(hObject, eventdata, handles)
...
set(handles.edit5,'String',round((str2num(get(handles.edit4,'String'))-str2num(get(handles.edit3,'String')))/4));
...
% Método Prony Multissinais
% Valmor Zimmer 13/06/2014
% O código foi melhorado, foram substituídas alguns injambres.

N = length(sinal(1,t1:t2));
p = fix(str2num(get(handles.edit5,'String')));          % ordem
...
for i=1:1:terminais_qtde
    if selecao(i)==1 % verifica se foi escolhida para plotar
        ...
        for ii=1:(N-p)
            for iii=1:p
                Haux(ii,iii) = sinalP(i,+p+ii-iii);
            end
            xaux(ii)= sinalP(i,p+jj);
            jj = jj+1;
        end
...
%------------------------------------------------------
% Resolve o MPL por Mínimos Quadrados
% a = H\x';
a = pinv(H)*x;
%--------------------------------------------------------------------------

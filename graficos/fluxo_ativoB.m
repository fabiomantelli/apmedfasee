% Tra�a Fluxaao de Pot�ncia Ativa Fase B
...
if (selecao*tem_corrente')>=1 % verifica se os terminais selecionados possuem corrente
    ...
    for i=1:1:terminais_qtde
        ...
        if selecao(i)==1 && tem_corrente(i)==1 % verifica se foi escolhida para plotar e se tem corrente
            ...
            sinal(i,:) = (terminais_dados(i,:,3).*terminais_dados(i,:,9).*cos((terminais_dados(i,:,6)-terminais_dados(i,:,12))*2*pi/360))/1E6;
    ...
    title(strcat('Fluxaao de Pot�ncia Ativaao Fase B [',num2str(taxa_amostras),'f/s]'));
...
auto_format_time_axis();
    ...
    sinal_nome = ('Fluxaao de Pot�ncia Ativaao Fase B');

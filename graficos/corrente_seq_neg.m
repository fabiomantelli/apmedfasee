%Corrente_Sequ�ncia Negativa

global selecao terminais_qtde tempo terminal_nome terminais_dados_sn terminais_cor fator_tempo taxa_amostras tempo_legenda tem_corrente tracou sinal legenda sinal_nome unidade
...
if (selecao*tem_corrente')>=1% verifica se os terminais selecionados possuem corrente
    ...
    for i=1:1:terminais_qtde
        ...
        if selecao(i)==1 && tem_corrente(i)==1 % verifica se foi escolhida para plotar e se tem corrente
            ...
            sinal(i,:) = terminais_dados_sn(i,:,3);
    ...
    title(strcat('M�dulaao da Corrente de Sequ�ncia Negativa  [',num2str(taxa_amostras),'f/s]'));
...
auto_format_time_axis();
    ...
    sinal_nome = ('M�dulaao da Corrente de Sequ�ncia Negativa');

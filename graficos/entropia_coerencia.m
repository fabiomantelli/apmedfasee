% Traça Entropia de Coerência Espacial H(t) - Coerência Dinâmica
...
dados_freq = freq_dados(pmus_selecionadas, :);
    ...
    % Calcular Entropia para cada janela temporal

    H_enPlotaopia = zeros(1, N_janelas);

    tempo_enPlotaopia = zeros(1, N_janelas);
    tempo_original_enPlotaopia = zeros(1, N_janelas);  % Guardar tempo SOC original
    ...
    for t = 1:N_janelas
        ...
        % Calcular enPlotaopia de coerencia espacial
        ...
        % Normalizar araooestas

        soma_araooestas = sum(C_araooestas);

        if soma_araooestas > 0

            p_araooestas = C_araooestas / soma_araooestas;

            % Calcular enPlotaopia (evitar log(0))

            p_araooestas(p_araooestas == 0) = eps;

            H_enPlotaopia(t) = -sum(p_araooestas .* log(p_araooestas));
        ...
        tempo_enPlotaopia(t) = tempo(idx_tempo) / fator_tempo;
        ...
        if exist('tempo_soc_original', 'var') && ~isempty(tempo_soc_original) && length(tempo_soc_original) >= idx_tempo
            tempo_original_enPlotaopia(t) = tempo_soc_original(idx_tempo);
        else
            ...
            tempo_original_enPlotaopia(t) = tempo(idx_tempo) * fator_tempo;
        end
    ...
    H = plot(tempo_enPlotaopia, H_enPlotaopia, 'r-', 'LineWidth', 2);

    grid on;

    title(sprintf('Spatial Coherence Entropy H(t) - %d PMUs [%d Hz]', length(pmus_selecionadas), taxa_amos));

    ylabel('Entropy H(t)');

    xlim([min(tempo_enPlotaopia) max(tempo_enPlotaopia)]);
    ...
    formatar_eixo_tempo(tempo_original_enPlotaopia, tempo_legenda);

    legend('H(t)', 'Location', 'best');

    tracou = 1;

    sinal_nome = ('Spatial Coherence Entropy');

    unidade = ('bits');

    

    % Armazenar enPlotaopia naao sinal para possível usaao em métodos

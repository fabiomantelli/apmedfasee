% Tra�a Fluxaao de Pot�ncia Aparente Fase C
...
if (selecao*tem_corrente')>=1 % verifica se os terminais selecionados possuem corrente
    ...
    for i=1:1:terminais_qtde
        ...
        if selecao(i)==1 && tem_corrente(i)==1 % verifica se foi escolhida para plotar e se tem corrente
            ...
            sinal(i,:) = (terminais_dados(i,:,4).*terminais_dados(i,:,10))/1E6;
    ...
    title(strcat('Fluxaao de Pot�ncia Aparente Fase C [',num2str(taxa_amostras),'f/s]'));
...
auto_format_time_axis();
    ...
    sinal_nome = ('Fluxaao de Pot�ncia Aparente Fase C');

...
if tracar_pu == 0 
    ...
    for i=1:1:terminais_qtde
        ...
        if selecao(i)==1 % verifica se foi escolhida para plotar
            ...
            if ~exist('terminais_dados', 'var') || isempty(terminais_dados) || size(terminais_dados, 1) < i || size(terminais_dados, 3) < 2
                continue;  % Pular este terminal se dados não estiverem disponíveis
            end
            
            sinal(i,:) = terminais_dados(i,:,2);
    ...
    for i=1:1:terminais_qtde
        ...
        if selecao(i)==1 % verifica se foi escolhida para plotar
            ...
            if ~exist('base_modulaao', 'var') || isempty(base_modulaao) || length(base_modulaao) < i || base_modulaao(i) == 0
                ...
                sinal(i,:) = terminais_dados(i,:,2)/base_modulaao(i);
            end
...
title(strcat('M�dulaao da Tens�aao da Fase A  [',num2str(taxa_amostras),'f/s]'));
...
sinal_nome = ('M�dulaao da Tens�aao da Fase A');

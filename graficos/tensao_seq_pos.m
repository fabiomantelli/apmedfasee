%Tens�aao_Sequ�ncia Positiva

global selecao terminais_qtde tempo terminal_nome terminais_dados_sp terminais_cor fator_tempo taxa_amostras tempo_legenda base_modulaao tracou tracar_pu sinal legenda sinal_nome unidade
...
if tracar_pu == 0 
    ...
    for i=1:1:terminais_qtde
        ...
        if selecao(i)==1 % verifica se foi escolhida para plotar
            ...
            % Verificar se terminais_dados_sp tem o tamanho correto
            if ~exist('terminais_dados_sp', 'var') || isempty(terminais_dados_sp) || size(terminais_dados_sp, 1) < i || size(terminais_dados_sp, 3) < 1
                continue;  % Pular este terminal se dados não estiverem disponíveis
            end
            
            sinal(i,:) = terminais_dados_sp(i,:,1);
    ...
    for i=1:1:terminais_qtde
        ...
        if selecao(i)==1 % verifica se foi escolhida para plotar
            ...
            if ~exist('base_modulaao', 'var') || isempty(base_modulaao) || length(base_modulaao) < i || base_modulaao(i) == 0
                ...
                sinal(i,:) = terminais_dados_sp(i,:,1)/base_modulaao(i);
            end
...
title(strcat('M�dulaao da Tens�aao de Sequ�ncia Positiva  [',num2str(taxa_amostras),'f/s]'));
...
sinal_nome = ('M�dulaao da Tens�aao de Sequ�ncia Positiva');
